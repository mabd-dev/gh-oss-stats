...
- **Main command** (default) - Fetch contribution stats from GitHub
- **`badge` sub-command** - Generate badges from existing stats JSON
...
The CLI supports several sub-commands for different use cases:

#### `badge` Sub-Command

Generate badges from existing stats JSON without re-fetching from GitHub.

**Purpose:**
- Create multiple badge variants from the same data
- Avoid GitHub API rate limits when generating many badges
- Process saved stats files locally
...
|-------|-------|-------------|
| --from-file | string | Path to stats JSON file (generated by main command) |
| --data | string | Stats as JSON string (inline JSON data) |
| + all badge configuration flags | | See [Badge Configuration Flags](#badge-configuration-flags) below |

**Examples:**

```bash
# Generate badge from a previously saved stats file
gh-oss-stats badge --from-file stats.json --badge-style summary --badge-theme dark

# Generate compact badge from stats file
gh-oss-stats badge --from-file stats.json --badge-style compact --badge-output my-badge.svg

# Generate badge from JSON string
gh-oss-stats badge --data '{"username":"mabd-dev","generatedAt":"2025-12-31T05:33:15Z",...}'

# Generate detailed badge with custom sorting
gh-oss-stats badge --from-file stats.json --badge-style detailed --badge-sort stars --badge-limit 10
```

**Status:** Stub implementation (not yet fully functional). This feature will be available in a future release.

#### `demo` Sub-Command

Generate demo badges using sample data for testing and previewing.

**Purpose:**
- Test badge styles and themes without fetching real data
- Preview how badges will look with realistic data
- Generate example badges for documentation
- Rapid prototyping of badge designs

**Flags:**

All badge configuration flags (see [Badge Configuration Flags](#badge-configuration-flags) below). No data input flags needed - uses built-in sample data.

**Examples:**

```bash
# Generate demo badge with summary style (dark theme)
gh-oss-stats demo --badge-style summary --badge-theme dark

# Generate demo badge with compact style
gh-oss-stats demo --badge-style compact --badge-output demo-compact.svg

# Generate detailed demo badge sorted by stars
gh-oss-stats demo --badge-style detailed --badge-sort stars --badge-theme nord

# Generate all theme variants of summary badge
gh-oss-stats demo --badge-style summary --badge-theme dark --badge-output demo-dark.svg
gh-oss-stats demo --badge-style summary --badge-theme light --badge-output demo-light.svg
gh-oss-stats demo --badge-style summary --badge-theme nord --badge-output demo-nord.svg
```
...
These flags control badge generation and styling. Used by the main command (with `--badge` flag) and sub-commands (`badge`, `demo`):
